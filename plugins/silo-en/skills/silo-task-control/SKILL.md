...
The panel follows the host Codex locale automatically and passes that locale to prompt preview and batch preparation. For conversational fallback outside the panel, reply in the user's current language. Never translate a user-edited prompt.
...
6. Send every claimed target its exact plan `prompt`. When `target.fast` is true or `target.modelOverride.serviceTier` is `fast`, call SILO's `send_task_prompt` with the exact target ID, prompt, model/effort overrides, `fast: true`, and the target's Burn flag; this is the only dispatch path that can faithfully request Codex Fast mode and surface approvals back inside SILO. Otherwise use the Codex app tool for sending a message to an existing task. A single confirmed Launch action authorizes these sends only. Use the target's `modelOverride` when present; otherwise use the plan-level `modelOverride`, and otherwise preserve the task's current model and thinking. A send tool returns only after the prompt has been accepted as a user-visible message in the destination task; immediately after that success, record status `running`, phase `sent`, and the matching `claimToken` so SILO can show `消息已显示在 Codex`. If Fast mode is unavailable and SILO reports a normal-tier fallback, record that fact in the phase without treating the send as failed. On send failure, record status `failed`, phase `dispatch`, the error, and the matching `claimToken`. Never resend merely because later visibility or monitoring checks are slow. A successful send is not completion.
