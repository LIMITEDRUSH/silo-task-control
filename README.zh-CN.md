# SILO Task Control

[English](README.md) | [简体中文](README.zh-CN.md)

SILO 是一个简约、轻量的 Codex 控制面板，用于跨项目扫描现有任务、逐项检查将要发送的继续或优化提示词，并通过 Codex 原生任务工具启动用户确认过的批次。

当前版本：**v0.6.2**（`0.6.2+codex.20260914`）

![SILO 任务控制面板总览](docs/images/silo-overview.png)

> 上方总览图是维护者提供的 SILO 在 Codex 中的实际运行截图。下方的聚焦对话和紧凑布局截图由当前 `ui/control.html` 配合虚构的本地 Mock Host 数据渲染。

## 界面放大说明

### 任务筛选与一键预设

![SILO 任务筛选、智能选择与 Launch](docs/images/silo-controls-zoom.png)

| 按钮 | 功能 |
|---|---|
| **全部 / 待处理 / 已完成** | 只切换当前任务视图，不修改选择，也不会启动任务。数字表示该分类中的任务数量。 |
| **智能选择** | 最多读取候选任务最近两轮对话，推荐适合继续或优化的任务；只给出建议，不直接运行。 |
| **🚀 Launch** | 按智能选择逻辑挑选任务，并一次性应用“继续 + Burn + 快速 + 模型和 Effort 跟随对话”。它是可撤销的预设，再按一次会取消由它加入的选择；仍需点击启动按钮确认。 |
| **任务行复选框** | 把该任务加入或移出当前批次。 |
| **任务标题** | 只打开右侧详情，不改变批次选择。 |
| **⋯** | 打开该任务的更多操作。 |

### 个人额度、刷新与启动

![SILO 额度、刷新与启动按钮](docs/images/silo-quota-controls-zoom.png)

| 控件 | 功能 |
|---|---|
| **额度卡片** | 显示 Codex 当前账户额度余额；Plus 用户同时显示 5 小时窗口，旁边显示正常额度窗口的下次刷新时间。 |
| **刷新面板** | 在当前 SILO 面板内重新扫描任务、运行状态和额度，不会新建对话或向其他任务发送 Prompt。 |
| **启动选中任务** | 打开批次确认窗口；只有任务已选择且 Prompt 就绪时才可点击。确认后才会真正调度。 |

### 右上角：Tibo 推特重置预测

![SILO Tibo 重置预测](docs/images/silo-reset-prediction-zoom.png)

这里显示的是**全局善意重置预测**：SILO 打开时从第三方 [codex-reset.com](https://codex-reset.com/zh/) 读取上次已验证重置和下次预测。该站综合追踪 Tibo（[@thsottiaux](https://x.com/thsottiaux)）的 X/Twitter 动态、已验证事件和历史节奏；点击这一区域可进入完整预测、时间线与来源。

它与左侧的个人额度卡片不同：个人额度卡片来自当前 Codex 账户窗口；Tibo 区域是公开的全局重置信号。如果没有可信的未来窗口，SILO 会显示“暂无计划”。这是第三方预测，不是 OpenAI 的官方承诺。

### 对话、模式与单任务控制

![SILO 对话与单任务操作](docs/images/silo-task-actions-zoom.png)

| 按钮 | 功能 |
|---|---|
| **对话** | 查看历史消息，并像在 Codex 中一样直接发送后续 Prompt。 |
| **文件** | 浏览对话涉及和工作目录中的文本文件，并在面板内预览。 |
| **历史** | 查看 SILO 批次及状态变化记录。 |
| **运行中** | 汇总当前 Codex 正在运行或等待处理的任务，并持续刷新状态。 |
| **继续工作** | 基于原对话上下文继续执行，并生成对应的 Continue Prompt。 |
| **优化结果** | 对已有成果进行质量复核与改进，并生成 Optimize Prompt。 |
| **模型 / Effort** | 默认跟随原对话，也可只为当前任务覆盖模型或推理强度。 |
| **⚡ 快速** | 为当前发送请求 Codex Fast service tier。 |
| **🔥 Burn** | 追加吞吐优先的并行执行策略；开启 Burn 时会同时打开快速模式，但不会跳过验证和安全边界。 |
| **发送 / 立即发送** | 将当前输入或已经审核的执行 Prompt 发送给原 Codex 任务。 |
| **×** | 关闭详情并返回任务列表。 |

## SILO 能帮你做什么

SILO 把最常用的任务操作集中在同一个工作台：找到真正需要处理的任务，决定继续还是优化，检查实际 Prompt 与运行设置，最后经确认再启动。

### 扫描、理解并选择任务

任务列表按项目整理对话，同时展示实时状态、模式、实际模型、推理强度与更新时间。**智能选择**会读取最近上下文并推荐一个聚焦的批次；**🚀 Launch**只应用这份推荐与一键设置，不会全选，也不会立刻启动。顶部还会持续显示额度余额和预计重置时间，不挤占主要工作区。

### 在完整上下文中继续或优化

![SILO 对话与任务控制](docs/images/silo-conversation.png)

选择任务后，右侧会直接呈现最近的用户与 Codex 对话。你可以切换**继续工作**或**优化结果**，修改系统生成的 Prompt，并在发送前独立调整 Burn、快速模式、模型和推理强度。文件、历史与运行中页面仍绑定同一个任务，检查资料时不会丢失当前位置。

### 窄面板中保持同一套工作流

![SILO 响应式紧凑面板](docs/images/silo-compact.png)

窗口变窄后，任务总览会让位给聚焦的单任务界面；搜索、任务筛选、模式选择、对话和启动入口仍然可达。Windows 与 macOS 使用同一套响应式面板和功能。

## 安装

先把 GitHub 仓库加入 Codex marketplace，再安装 SILO：

```sh
codex plugin marketplace add LIMITEDRUSH/silo-task-control
codex plugin add silo-cn@silo-plugins
# 原英文包名也继续支持：
codex plugin add silo-en@silo-plugins
```

如果需要保持 v0.6.2 工作台外观、同时适配 Codex 内置面板的独立 macOS 包：

```bash
codex plugin add silo-mac@silo-plugins
```

首次安装后重启 Codex。可以从侧边栏打开 **SILO**，也可以新建任务后输入：

```text
$silo-task-control 打开控制面板，扫描我现在的任务。
```

两个包名现在采用相同的自动本地化逻辑。SILO 会优先读取本地 Codex 的语言，再回退到浏览器/系统语言；界面以及“继续/优化”生成的提示词会保持同一种语言，面板内无需语言开关。`silo-cn` 和 `silo-en` 继续保留，以兼容已有安装和升级。

后续更新：

```sh
codex plugin marketplace upgrade silo-plugins
codex plugin add silo-cn@silo-plugins
```

SILO 支持 Windows 与 macOS，需要 Codex Desktop 和 Node.js 22.5 或更高版本。两个平台运行同一个 MCP 服务包和同一个响应式 `control.html`，因此 Codex 内置界面与功能保持一致。

macOS 还需要安装当前版本的 [Codex CLI](https://developers.openai.com/codex/cli)，确保终端中可以运行 `codex`。SILO 能识别官方独立安装使用的 `~/.local/bin`，也能识别常见 Homebrew 路径。

## 主要能力

- 快速读取本地任务，并结合 Codex 原生状态同步与 SQLite 后备索引。
- 自动识别本地 Codex 语言，并在简体中文和英文界面之间同步切换。
- 简洁的 **全部 / 待处理 / 已完成 / 智能选择** 工作流、全局搜索和项目分组。
- 可撤销的 **🚀 Launch** 一键预设：按智能选择结果加入任务，设置继续、Burn、快速以及跟随原对话的模型/Effort，但不会立即启动。
- 逐任务检查完整提示词，支持继续/优化、独立 Burn 策略、模型、推理强度及显式启动确认。
- 使用持久化原生 job、原子 claim 和进度恢复构建实时运行栏；面板或服务重启后仍可恢复状态。
- 轻量额度轮询和重置预测。面板打开时会从第三方服务 [codex-reset.com](https://codex-reset.com/zh/) 获取重置信息。
- 键盘操作：`/` 聚焦搜索，方向键移动任务，`X` 切换选择，`Space` 打开详情，`Esc` 清空批次但保留草稿。

## 调度流程

打开、扫描、筛选和智能选择均为只读。智能选择最多检查候选任务最近两轮对话，只提出继续或优化建议，不会直接启动任务。

启动必须通过确认对话框。SILO 会保存短时有效的计划，发送一条可见的跟进消息，通过 Codex 原生任务工具重新读取每个目标，跳过正在运行或需要用户处理的任务，为每个可用目标执行原子 claim，并把进度写入运行栏。它不会创建、派生、归档、重命名或中断无关任务。

## 外置面板

在 SILO 中选择 **外置窗口**。Windows 也可双击 `desktop\Start SILO.cmd`；macOS 可打开 `desktop/Start SILO.command`。启动器会启动只监听本机回环地址的服务，并用应用窗口打开同一面板。macOS 优先使用 Chrome、Edge、Brave 或 Chromium，没有 Chromium 浏览器时回退到默认浏览器。Codex CLI 会依次从 `CODEX_CLI_PATH`、`PATH` 和常见 Homebrew 路径中查找。

SILO 的临时桌面运行记录在 Windows 保存到 `%LOCALAPPDATA%\SILO`，在 macOS 保存到 `~/Library/Application Support/SILO`。任务历史在两个平台都继续读取本地 Codex 数据目录（`CODEX_HOME` 或 `~/.codex`）。

## 侧边栏入口

SILO 会把 MCP App 声明为全局入口，因此兼容的 Codex Desktop 版本可在侧边栏显示 **SILO**。这项宿主元数据目前仍未成为公开 API。Codex Desktop 更新后可运行：

```sh
npm run sidebar-doctor
```

该命令只进行兼容性检查。SILO 不会修补 Codex，也不会修改 `app.asar`。

## 开发

```sh
npm install
npm run check
```

打包后的 MCP 服务入口为 `dist/server.mjs`。

## 安全边界

- 扫描与计划创建均为只读。
- 计划 15 分钟后失效，只包含最近扫描到的任务 ID。
- 调度需要显式确认，并会在 Codex 中显示可见消息。
- 编辑后的提示词按审核内容发送；启用 Burn 时只追加一次。
- 每个目标都会在发送前通过原生任务 API 再次检查。
- 外置面板的 STOP 只影响由该 SILO 进程启动的子任务。

## 许可证

MIT
