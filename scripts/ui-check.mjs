...
assert.match(html, /id=["']normalQuotaReset["']/);
assert.match(html, /id=["']plusQuotaResetRow["']/);
assert.match(html, /\.quota-reset\[hidden\]\{display:none\}/);
assert.match(html, /function formatQuotaReset\(value\)/);
assert.match(html, /function quotaWindowInfo\(rate\)/);
assert.match(html, /Math\.abs\(duration-300\)<=30/);
assert.match(html, /\/plus\/i\.test/);
...
assert.match(html, /id=["']quickLaunch["'][^>]*>🚀 Launch<\/button>/);
assert.match(html, /views\.appendChild\(els\.quickLaunch\)/);
assert.match(html, /function applyQuickLaunchPreset\(\)/);
assert.match(html, /name:"recommend_tasks",arguments:\{limit:Math\.min\(8,selectionLimit\(\)\)\}/);
assert.match(html, /state\.selected\.set\(task\.id,"continue"\);state\.burn\.add\(task\.id\);state\.fast\.add\(task\.id\);state\.taskModels\.set\(task\.id,"preserve"\);state\.taskThinking\.delete\(task\.id\)/);
assert.match(html, /state\.launchPresetIds\.add\(task\.id\)/);
assert.match(html, /state\.launchPresetIds\.size/);
assert.match(html, /els\.quickLaunch\.setAttribute\("aria-pressed",active\?"true":"false"\)/);
assert.match(html, /尚未启动/);
assert.match(html, /\.view-switch>\.quick-launch/);
assert.match(html, /\.view-switch>\.quick-launch\[aria-pressed="true"\]/);
...
assert.match(html, /\.conversation-modes\{gap:3px;padding:3px;border:1px solid var\(--border\);border-radius:10px/);
assert.match(html, /\.conversation-modes button\{min-height:32px;padding:5px 12px;border:0!important;border-radius:7px!important/);
assert.match(html, /effortChoices=\[\["preserve","跟随对话","使用此对话当前推理强度"\]/);
assert.match(html, /effortValue=state\.taskThinking\.has\(task\.id\)\?state\.taskThinking\.get\(task\.id\):"preserve"/);
