...
const readControlHtml = () => readFileSync(CONTROL_HTML_PATH, "utf8");
...
const scanInputShape = {
  ...
  liveThreads: z.array(liveThreadShape).max(50).default([]),
};
...
registerAppResource(server, "silo-task-control", CONTROL_URI, {}, async () => {
  const html = readControlHtml().replaceAll(
    "__SILO_BUILD_VERSION__",
    VERSION,
  );
  return {
    contents: [
